...
interface MatchBrowserTabProps {
  events: Plus2Event[];
...
export default function MatchBrowserTab({
  events,
  goalsByMatch,
  teams,
  seasons,
}: MatchBrowserTabProps) {
  // Internal state
  ...
  // Helpers that resolve from teams prop
  ...
  const sortedSeasons = useMemo(
    () => seasons.slice().sort((a, b) => a.sort_order - b.sort_order),
    [seasons],
  );
  ...
  const filteredEvents = useMemo(() => {
    ...
    return list;
  }, [events, seasonFilter, resultFilter, venueFilter, searchText]);
